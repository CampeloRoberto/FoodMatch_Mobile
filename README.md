...
Para melhorar sua experiência de desenvolvimento ao rodar o projeto, aqui vão algumas dicas essenciais:

Limpando o Cache: O cache do Metro Bundler e dos emuladores às vezes pode causar comportamentos inesperados (é importante limpar de vez em quando!). Para iniciar a aplicação com o cache zerado, utilize a flag --clear:

npx expo start --clear

Descobrindo o erro "Something went wrong": Se você estiver testando o app pelo Expo Go no celular e se deparar com a tela genérica "Something went wrong" sem saber o que causou o crash, faça o seguinte:

Com a aplicação ainda rodando, vá ao terminal.

Digite a tecla w para abrir a aplicação no navegador web.

Geralmente, fazer isso força o Expo a exibir o código de erro exato e o stack trace no terminal, facilitando muito a vida na hora de debugar.
